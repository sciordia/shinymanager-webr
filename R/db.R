...
add_user <- function(dbdir, email, password,
  ...
  with_con(dbdir, function(con) {
    DBI::dbExecute(
      con,
      "INSERT INTO users (user_id, email, name, role, password_hash, pushover_user_key_enc, profile, created_at)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?)",
      params = list(user_id, email, name %||% NA_character_, role,
                    hash_password(password), pk_enc, profile_json, now_epoch())
    )
  })
  invisible(user_id)
}
